...
消费方通过使用点的 `service` prop，或 `app.use(Admin9UI, { fileService })` 注入 adapter。

- `ATiptapEditor` 的图片、视频和音频资源选择统一使用 `AFilePicker`，通过 `FilePickerAdapter` 或 `fileService` 提供。
...
`AFilePicker` 用于表单、弹窗和附件字段中的完整选择工作流；`AFileUploader` 提供可独立使用的本地批量上传队列，并由 Picker 组合复用。上传完成会刷新当前列表，但不会自动改变 Picker 草稿，上传中也可继续追加文件。文件契约支持 `image`、`video`、`audio`、`document`、`archive`、`other` 六种真实 `FileType`；“全部”只是聚合查询筛选，不会写入 `FileItem` 或传给上传能力。具体 `FileType` 是上传目标分类；默认不会限制本地选择格式，宿主可按需传入 `accept`。

消费方通过使用点的 `service` prop，或 `app.use(Admin9UI, { fileService })` 注入 `FilePickerAdapter`；使用点 prop 优先。文件管理页面及其删除、移动、分组管理、权限和业务字段属于消费应用。

`FileListParams` 的聚合查询省略 `fileTypes` 表示六类全部；提供 `fileTypes` 时表示准确的真实类型集合，空数组表示无匹配。adapter 必须在后端对完整集合进行筛选和分页，并返回准确 `pagination.total`，禁止仅过滤当前页。具体类型查询才能携带 `groupId`。完整行为见 [`AFilePicker` 文档](./docs/components/file-picker.md)与 [`AFileUploader` 文档](./docs/components/file-uploader.md)。
...
4. 创建规范 tag，例如 `git tag -a v0.5.0 <release-commit> -m "v0.5.0"`，再次确认 tag、package 版本、CHANGELOG 和提交后，单独推送该 tag。一次只能推送一个发布 tag，必须等该 workflow 完成后再推送下一版本；GitHub concurrency 只保留一个 running 和一个 pending，新的 pending 可能替换旧的 pending。
