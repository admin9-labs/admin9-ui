...
| `accept`      | `string`                              | `undefined`        | 仅传给原生文件选择器的 MIME/扩展名提示                   |
...
| `upload-success`    | `FileItem`                            | 当前视图内上传成功，即使返回项因资格不符未自动选择 |
...
- 外部模型和上传响应使用同一资格校验。上传 ID 与当前列表或任一跨页草稿冲突时不自动选择；达到 limit 时上传仍可成功，但不改变草稿。
...
- 上传只在当前选择真实 `FileType` 时启用；聚合“全部”永不把 `undefined` 或 `all` 传给 `upload`。
