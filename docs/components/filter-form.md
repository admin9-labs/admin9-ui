...
组件不包含卡片、标题和分隔线。消费方应把它放入列表页已有的内容容器，并将 `search`、`reset` 与自己的分页和数据请求衔接。
...
筛选项的业务默认值和重置后的查询时机由列表页决定。组件不会调用 `resetFields`。组件没有 `mode`、`collapsible`、`collapsedRows` 或折叠状态 `v-model`。折叠能力只在字段超过两行时出现，收起后固定显示第一行；提交校验失败时会自动展开，确保错误字段可见。
