...
`AFileUploader` 是后端无关的本地批量上传组件。它基于现有单文件 `FileUploadCapability` 组织队列，统一处理进度、取消、失败重试、部分成功、结果校验和异步生命周期，可独立使用，也由 `AFilePicker` 与 `AFileManager` 复用。
...
| 事件           | 参数                                     | 时机                                                                                             |
| -------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `response`     | `(item: FileItem, task: FileUploadTask)` | adapter Promise 已解析；为 Picker/Manager 旧 `upload-success` 语义保留，结果可能尚未通过资格校验 |
...
- `AFileManager` 在队列完成后刷新当前类型和分组，不把上传结果加入管理选择。
- 两者继续保留原有单项 `upload-success` / `upload-error` 事件兼容层，但不再维护各自的上传请求和进度状态。
